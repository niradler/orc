...
export function ChatPanel({ open, onToggle }: ChatPanelProps) {
  ...
  if (!open) {
    return (
      <div className="fixed top-0 right-0 h-full w-12 z-40 bg-surface border-l border-surface-highest flex flex-col items-center pt-4 gap-3">
        <button
          onClick={onToggle}
  ...
  return (
    <div className="fixed top-0 right-0 h-full w-80 z-40 flex flex-col bg-surface border-l border-surface-highest transition-all duration-200">
      {/* Header */}
      ...
      >
        ...
        {messages.map((msg, i) => (
          <div key={i}>
            ...
            <p className="text-on-surface font-mono text-xs mt-0.5 whitespace-pre-wrap leading-relaxed">
              {msg.content}
            </p>
          </div>
        ))}
