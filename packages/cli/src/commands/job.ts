...
export function jobCommand() {
  ...
  cmd
    ...
    .option("--trigger <type>", "Trigger type (manual/cron/repeat/watch)", "manual")
    .option("--cron <expr>", "Cron expression (e.g. '0 22 * * *')")
    .option("--every <secs>", "Repeat interval in seconds")
    ...
    .action(async (name: string, opts) => {
      ...
      const { data, error } = await client.jobs.create({
        ...
        trigger_type: opts.trigger as
          ...
          | "bridge-msg",
        cron_expr: opts.cron,
        ...(opts.every ? { repeat_secs: Number(opts.every) } : {}),
      ...
      for (const r of runs) {
        ...
        const dur =
          r.started_at && r.ended_at
            ...
            : "-";
        console.log(`${icon} ${r.status.padEnd(10)} ${r.created_at.slice(0, 19)}  ${dur}`);
      }
    });
